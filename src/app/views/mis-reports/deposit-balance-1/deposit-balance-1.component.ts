...
@Component({
  selector: 'app-deposit-balance-1',
  templateUrl: './deposit-balance-1.component.html',
  styleUrls: ['./deposit-balance-1.component.css'],
  providers: [
    { provide: DateAdapter, useClass: PickDateAdapter },
    { provide: MAT_DATE_FORMATS, useValue: PICK_FORMATS }
  ]
})
export class DepositBalance1Component implements OnInit {
  ...
  exportExcel() {
    ...
    this.http.downloadFile("/misreport/downloadDepositBalance1ReportFile", requestBody, `DepositBalance1_${this.branchCode}_${this.ccy}_${reportDate}`, 'xlsx').subscribe(
      (data: any) => {
        ...
        console.log("Deposit Balance 1 Excel Exporting Error >>> " + error)
        if (error != "") {
          this.error = "(The system cannot cannot export Deposit Balance 1 excel file!.. Have the error)";
        }
        this.loading = false;
      });
