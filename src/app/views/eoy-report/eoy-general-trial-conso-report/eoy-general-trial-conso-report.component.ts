...
@Component({
  selector: 'app-eoy-general-trial-conso-report',
  templateUrl: './eoy-general-trial-conso-report.component.html',
  styleUrls: ['./eoy-general-trial-conso-report.component.css'],
  providers: [
...
export class EOYGeneralTrialConsoReportComponent implements OnInit {
  error='';
  from_date:Date;
  loading = false;
  data:TrialReport;
  ...
  comboData.formtype = "1";

  this.http.doPost("/misreport/get_general_trial_report", comboData).subscribe( (res:TrialReport)=>{
    this.loading = false;
    if(res != null){
      this.data_message="";
      this.data = res;
...
this.http.downloadFile("/misreport/get_excel_general_trial_report", comboData,this.getFileName(),"xlsx").subscribe(  (data: any) => {
  this.loading = false;
    },error => {
      console.log("General Trial Report Excel Exporting Error >>> " + error)
      if (error != "") {
        this.error = "(The system cannot export General Trial Report excel file!.. Have the error)";
      }
      this.loading = false;
    });
  ...
  getFileName(){
    ...
    filename = 'GeneralTrial_'+bName+'_'+this.currencyCode+"_R"+this.roundSrno;
    return filename;
  }
...
this.http.downloadFile("/misreport/get_pdf_general_trial_report", comboData,this.getFileName(),"pdf").subscribe(  (data: any) => {
  this.loading = false;
    },error => {
      console.log("General Trial Report Pdf Exporting Error >>> " + error)
      if (error != "") {
        this.error = "(The system cannot export General Trial Report pdf file!.. Have the error)";
      }
      this.loading = false;
    });
